...
class AccountsCustom {
            ...
            if($get_request){
                ...
                if (array_key_exists('role', $param)) {
                    if(isset($param['role']) && !in_array($param['role'],[self::ROLE_CUSTOMER, self::ROLE_ENTREPRENEUR, self::ROLE_ADVERTISER])){
                        $result = array("code" => 4000, "description" => "Expected ENTREPRENEUR or CUSTOMER or ADVERTISER for key (role)");
                            echo json_encode($result, JSON_UNESCAPED_SLASHES);
                            return false;
                    }
                }else{
                    throw new Exception("Expected CUSTOMER or ADVERTISER or ENTREPRENEUR for key (role), " . (is_object($param['role']) ? get_class($param['role']) : gettype($param['role'])) . ' found.');
                }                                
                ...
                if (array_key_exists('email', $param)) {
                    ...
                    $account = new Account();
                    $email = $account::where('email', '=', $param['email'])->first();                                                                                                
                    if ($email) {
                        ...
                        $result = array("code" => 4001, "description" => "Email already exists");                       
                ...
                if (array_key_exists('role', $param)){
                    ...
                    if(isset($param['role']) && SELF::ROLE_GUEST === $this->_role){
                        if(!in_array($param['role'],[self::ROLE_CUSTOMER, self::ROLE_ENTREPRENEUR, self::ROLE_ADVERTISER])){
                            $result = array("code" => 4000, "description" => "Expected ENTREPRENEUR or CUSTOMER or ADVERTISER for key (role)");
                            echo json_encode($result, JSON_UNESCAPED_SLASHES);
                            return false;
                        }
                ...
                if (array_key_exists('password', $param)){
                    ...
                    if (!is_string($param['password'])) {
                        $result = array("code" => 4000, "description" => "secret must be a string");
                        echo json_encode($result, JSON_UNESCAPED_SLASHES);
                        return false;
                    }
    ...
    public function dbSave($params) {
        try {                        
            ...
            if($saved){             
                ...
                /*
                Mail::send('email.verify',$params['verify_token'],function((){
    ...
    public function verify($email, $token) {
        try {
            $result = null;            
            $account = new Account();       
            $result = $account::where('email', '=', $email)->where('verify_token', '=', $token)->first();           
            return $result;
        } catch (Exception $ex) {
            LogRepo::printLog('error', $ex->getMessage());
        }
